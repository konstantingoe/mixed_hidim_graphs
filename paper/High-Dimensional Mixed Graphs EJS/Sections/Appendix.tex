...
%Denote $\Xi(t) = P((X_k - \mu_k)/\sigma_k \leq t)$ and set $\lambda(\Xi; x^j, \xi^j) = \sum_{r=1}^{l_{X_j}} x^r_j \int_{\xi_{r-1}^{j}}^{\xi_{r}^{j}} z d\Xi(z)$ where $\xi^j = (\xi_j^0, \xi^j_1, \dots, \xi_j^{l_j +1})^\prime$ and $\xi^r_j = \Xi^{-1}(p_{\cdot r}^j)$ then by \citet[p. 430]{Bedrick96} we obtain
...
%\begin{equation*}
%    \lambda(\tilde{\Xi}; x^j, \hat{\xi}^j) = \sum_{r=1}^{l_{X_j}} x^r_j \int_{\hat{\xi}_{r-1}^{j}}^{\hat{\xi}_{r}^{j}} z d\tilde{\Xi}(z) = \frac{1}{n\hat{\sigma}_k}\sum_{r=1}^{l_{X_j}} x^r_j \sum_{c=1+np_{\cdot r-1}^j}^{np_{\cdot r-1}^j}(X_{k(c)} - \Bar{X}_k),
%\end{equation*}
...
\end{enumerate}
    ...
    \begin{condition}[Population risk is strongly Morse]
        ...
        \begin{align*}
            ...
            &= 0,
        \end{align*}
        since all terms except the ones involving $\phi({\tilde{\Gamma}}^j_{0})$ and $\phi({\tilde{\Gamma}}^j_{l_j+1})$ cancel and furthermore $ \lim\limits_{t \to \pm \infty} \phi(t) = \lim\limits_{t \to \pm \infty} \phi^\prime(t) = 0$ .
...
With these considerations, we have verified the required four conditions to hold such that Theorem 3.2\todo{Rather cite Theorem 2 in the non-convex loss paper here.}
%\ref{uniform_convergence}%
holds for each couple $(j,k)$ with $j < k$. More precisely, let $\alpha \in (0,1)$. Now, letting  $n \geq 4 C \log(n) \log(\frac{B}{\alpha})$
...
\begin{lemma}\label{interchange_caseII_firstdif}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \begin{split}
                \int_S (1-(\Sigma_{jk})^2)^{-\frac{3}{2}}&\Big[\phi({\tilde{\Gamma}}_j^{r})({\Gamma}_j^r\Sigma_{jk} - {\tilde{x}}_{k}) - \phi({\tilde{\Gamma}}_j^{r-1})({\Gamma}_j^{r-1}\Sigma_{jk} - {\tilde{x}}_{k})\Big] d\mu(x^r_j) = \\
            ...
            \end{split}
        \end{equation*}
        since all terms except the ones involving $\phi({\tilde{\Gamma}}^j_{0})$ and $\phi({\tilde{\Gamma}}_j^{l_j +1})$ cancel and 
        \begin{equation*}
...
\begin{corollary}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            ...
            \end{split}
        \end{equation*}
        By similar arguments to Lemma \ref{interchange_caseII_firstdif}, when taking the sum over all possible states all terms except the ones involving $\phi({\tilde{\Gamma}}^j_{0})$ and $\phi({\tilde{\Gamma}}_j^{l_j +1})$ still cancel as they appear in every additive term in the above equation -- recall that $\phi^\prime(t) = -t\phi(t)$ -- and equality then follows immediately. 
    \end{proof}
\end{corollary}    
    ...
    \begin{enumerate}
        \item \begin{equation*}
               ...
               \end{equation*}
                i.e. 
               \begin{align*}
                    \frac{\partial }{\partial \Sigma_{jk}} \int_{S\times \mathbb{R}} \log L(\Sigma_{jk}, x^r_j,x_k) L(\Sigma_{jk}^*, x^r_j,x_k) d\varepsilon(x^r_j,x_k) = \\
                    \int_{S\times \mathbb{R}} \frac{\partial }{\partial \Sigma_{jk}} \log L(\Sigma_{jk}, x^r_j,x_k) L(\Sigma_{jk}^*, x^r_j,x_k) d\varepsilon(x^r_j,x_k)
               \end{align*}
...
First, note that $\Phi^{-1}(\cdot)$ is Lipschitz on $[\Phi(-2G),\Phi(2G)]$ with a Lipschitz constant $L_1$ such that under the event $A^r_j = \big\{\abs{\hat{\Gamma}_j^r} \leq 2G\big\}$
\begin{equation*}
    \begin{split}
        \abs{\hat{\Gamma}^r_j - \Gamma^r_j} &= \abs{\Phi^{-1}\left(\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} \leq x^r_j)}\right) - \Phi^{-1}\left(\Phi\left(\Gamma^r_j\right)\right)} \\
        &\leq L_1\abs{\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{\left(X_{ji} \leq x^r_j\right)} - \Phi\left(\Gamma^r_j\right)}.
    \end{split}
\end{equation*}
...
\begin{split}
    P\left({A^c}_{r}^{j}\right) &= P\Big(\abs{\hat{\Gamma}_j^r} > 2G\Big) \\
                     ...
                     &\leq P\left(\abs{\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} \leq x^r_j)} - \Phi(\Gamma^r_j)} > \frac{G}{L_1}\right) \\ 
                     &\leq 2\exp{\Big(- \frac{2G^2n}{L_1^2}\Big)},
\end{split} 
\end{equation*}
where the last step follows from Hoeffding's inequality. Further define the event $A^j = \bigcap_{r=1}^{l_{X_j}-1} A^r_j$ and observe that
\begin{equation*}
    P\left({A^c}^j\right) = P\left(\bigcup_{r=1}^{l_{X_j}-1} {A^c}^r_j\right) \leq \sum_{r=1}^{l_{X_j}-1} P({A^c}^r_j) \leq 2(l_{X_j}-1)\exp{\Big(- \frac{2G^2n}{L_1^2}\Big)},
\end{equation*}
as desired. 
...
\begin{lemma}
    \begin{proof}
        Recall that
        \begin{equation}
            \hat{\gamma}^r_j = \Phi^{-1}(p_{\cdot r}^j),
        \end{equation}
        where $p_{\cdot r}^j$ denotes the proportion of observing $x^r_j$ in an n-sample of the random variable $X_j$ where $1 < j < d_1+1$, i.e.
        \begin{equation*}
            p_{\cdot r}^j = \frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x^r_j)}.
        \end{equation*}
        First, note that $\Phi^{-1}(\cdot)$ is Lipschitz on $[\Phi(-2G),\Phi(2G)]$ with a Lipschitz constant $L_1$ such that under the event $A^r_j = \{\abs{\hat{\gamma}_j^r} \leq 2G\}$
        \begin{equation*}
            \begin{split}
                \abs{\hat{\gamma}^r_j - \gamma^r_j} &= \abs{\Phi^{-1}(\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x^r_j)}) - \Phi^{-1}(\Phi(\gamma^r_j))} \\
                &\leq L_1\abs{\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x^r_j)} - \Phi(\gamma^r_j)}.
            \end{split}
        \end{equation*}
        ...
        \begin{equation*}
            \abs{\phi(\hat{\gamma}^r_j) - \phi(\gamma^r_j)} \leq L_0 \abs{\hat{\gamma}^r_j - \gamma^r_j} \leq \abs{\hat{\gamma}^r_j - \gamma^r_j},
        \end{equation*}
        as $L_0 < 1$. 
        We may then bound the probability of the complementary event as
        \begin{equation*}
        \begin{split}
            P({A^c}_{r}^{j}) &= P\Big(\abs{\hat{\gamma}_j^r} > 2G\Big) \\
                             ...
                             &\leq P\Big(\abs{\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x^r_j)} - \Phi(\gamma^r_j)} > \frac{G}{L_1}\Big) \\ 
                             &\leq 2\exp{\Big(- \frac{2G^2n}{L_1^2}\Big)},
        \end{split} 
        \end{equation*}
        where the last step follows from Hoeffding's inequality. Further define the event $A^j = \bigcap_{r=1}^{l_{X_j}-1} A^r_j$ and observe that
        \begin{equation*}
            P({A^c}^j) = P\Big(\bigcup_{r=1}^{l_{X_j}-1} {A^c}^r_j\Big) \leq \sum_{r=1}^{l_{X_j}-1} P({A^c}^r_j) \leq 2(l_{X_j}-1)\exp{\Big(- \frac{2G^2n}{L_1^2}\Big)}.
        \end{equation*}
    \end{proof}
\end{lemma}    
