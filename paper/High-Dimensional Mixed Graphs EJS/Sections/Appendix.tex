...
%We need to find sample equivalences for each component in Eq. \eqref{eq:polyserial_adhoc}. As the numerator does not change compared to the naive approach, let us focus on $\lambda(\Xi; x^j, \xi^j)$. First, let $\tilde{\Xi}(t) = 1/n\sum_{i=1}^n \mathbbm{1}_{(X_k - \mu_k)/\sigma_k \leq t}$ be the empirical distribution function of the standardized observed continuous variable $X_k$. In order to estimate the threshold parameters $\xi^j$ we need to evaluate percentiles of $\tilde{X}_k$, i.e, $\hat{\xi}_r^j = \tilde{\Xi}^{-1}(p_{\cdot r}^j)$. The $X_k$'s are unique, with probability tending to one. Hence, we can estimate the corresponding percentiles using order statistics, i.e.
%\begin{equation*}
...
%\begin{equation*}
%    \lambda(\tilde{\Xi}; x^j, \hat{\xi}^j) = \sum_{r=1}^{l_{X_j}} x_r^j \int_{\hat{\xi}_{r-1}^{j}}^{\hat{\xi}_{r}^{j}} z d\tilde{\Xi}(z) = \frac{1}{n\hat{\sigma}_k}\sum_{r=1}^{l_{X_j}} x_r^j \sum_{c=1+np_{\cdot r-1}^j}^{np_{\cdot r-1}^j}(X_{k(c)} - \Bar{X}_k),
%\end{equation*}
...
With regard to third condition we introduce some additional notation. Let the sample risk be denoted by $\hat{R}_n(\Sigma_{jk})$. In order to avoid too many subscripts, $\hat{R}_n(\Sigma_{jk})$ represents the sample risk both in \textit{Case II} and case III, i.e. 
\begin{equation*}
...
\begin{condition}[Hessian regularity]
    ...
    \begin{condition}[Population risk is strongly Morse]
        ...
        \begin{align*}
            ...
            \frac{\partial^2 p(x_j^{r} \mid x_{k}, \Sigma_{jk})/\partial \Sigma_{jk}^{2}}{p(x_j^{r} \mid x_{k}, \Sigma_{jk})} - \Bigg(\frac{\partial p(x_j^{r} \mid x_{k}, \Sigma_{jk}) / \partial \Sigma_{jk}}{p(x_j^{r} \mid x_{k}, \Sigma_{jk})}\Bigg)^2 & \text{for case II}, \\
        ...
        Note, that for \textit{Case II} we have
        \begin{multline*}
                \mathbb{E}_{\Sigma_{jk}^*}\Bigg[\frac{\partial^2 p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)/\partial \Sigma_{jk}^{2}}{p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)}\Bigg] \\
                \begin{aligned}
                &= \int_{-\infty}^{\infty} \sum_{r=1}^{l_{l_{X_j}}} \frac{\partial^2 p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)/\partial \Sigma_{jk}^{2}}{p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)} p(x_j^{r}, x_{k}; \Sigma_{jk}^*) dx_k \\
                &= \int_{-\infty}^{\infty} \sum_{r=1}^{l_{l_{X_j}}} \frac{\partial^2 p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)/\partial \Sigma_{jk}^{2}}{p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)} p(x_j^{r} \mid x_{k}, \Sigma_{jk}^*)p(x_{k}) dx_k \\
...
\begin{lemma}
    \begin{proof}
        ...
        \end{equation*}
        First, note that $\Phi^{-1}(\cdot)$ is Lipschitz on $[\Phi(-2G),\Phi(2G)]$ with a Lipschitz constant $L_1$ such that under the event $A_r^j = \{\abs{\hat{\gamma}_j^r} \leq 2G\}$
        \begin{equation*}
            \begin{split}
            ...
            \end{split}
        \end{equation*}
        Now, we are interested in $\phi(\hat{\gamma}_j^r)$, therefore note further that $\phi(\cdot)$, namely the density function of the standard normal is Lipschitz with Lipschitz constant $L_0 = (2\pi)^{-1/2}e^{-1/2}$, s.t.
        \begin{equation*}
        ...
        \begin{equation*}
        \begin{split}
            P({A^c}_{r}^{j}) &= P\Big(\abs{\hat{\gamma}_j^r} > 2G\Big) \\
                             &= P\Big(\abs{\hat{\gamma}_j^r} - \abs{\gamma_j^r} > 2G - \abs{\gamma_j^r}\Big) \\
                             &\leq P\Big(\abs{\hat{\gamma}_j^r} - \abs{\gamma_j^r} > G\Big) \quad \text{(Assumption \ref{ass2})} \\
                             &\leq P\Big(\abs{\hat{\gamma}_j^r - \gamma_j^r} > G\Big) \quad \text{(reverse triangle inequality)} \\
...
\begin{equation*}
    \sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}_j^r)(x^j_{r+1} - x_j^r).
\end{equation*}
...
\begin{multline*}
    ...
    \begin{aligned}
    &\leq P\left( \max_{jk} \abs{\frac{S_{\hat{f}(X_k)X_j}}{\sigma^{(n)}_{\hat{f}(X_k)}\sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}_j^r)} - \frac{S_{{f}(X_k)X_j}}{\sigma^{(n)}_{{f}(X_k)}\sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}_j^r)}} \geq \epsilon \right) \\
    &+ P\left(\max_{jk} \abs{\frac{S_{{f}(X_k)X_j}}{\sigma^{(n)}_{{f}(X_k)}\sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}_j^r)} - \Sigma_{jk}^{*}} \geq \epsilon \right) \\
