...
Similar to case II above, we employ the \textit{two-step estimator} for the polychoric correlation. Given the threshold estimates from the first step, let us state the derivative of $\ell^{(n)}(\Sigma_{jk}, x^j_r,x^k_s)$ with respect to $\Sigma_{jk}$ explicitly. First, recall that from Eq. (5) %\eqref{cell_probabilities}%
\begin{equation}
    \begin{split}
...
Thus, taking the derivative of $\ell^{(n)}(\Sigma_{jk}, x^j_r,x^k_s)$ with respect to $\Sigma_{jk}$ yields
\begin{multline*}{2}
...
%\section{Case II under the Gaussian Copula model}
...
%Denote $\Xi(t) = P((X_k - \mu_k)/\sigma_k \leq t)$ and set $\lambda(\Xi; x^j, \xi^j) = \sum_{r=1}^{l_{X_j}} x_r^j \int_{\xi_{r-1}^{j}}^{\xi_{r}^{j}} z d\Xi(z)$ where $\xi^j = (\xi^j_0, \xi^j_1, \dots, \xi^j_{l_{X_j}})^\prime$ and $\xi_r^j = \Xi^{-1}(p_{\cdot r}^j)$ then by \citet[p. 430]{Bedrick96} we obtain
%\begin{equation}\label{eq:polyserial_adhoc}
%    \Sigma_{jk}^* = \frac{\rho_{X_j X_k} \sigma_j}{\lambda(\Xi; x^j, \xi^j)}.
%\end{equation}

%We need to find sample equivalences for each of the components in Eq. \eqref{eq:polyserial_adhoc}. As the numerator does not change as compared to the naive approach let us focus on $\lambda(\Xi; x^j, \xi^j)$. First, let $\Tilde{\Xi}(t) = 1/n\sum_{i=1}^n \mathbbm{1}_{(X_k - \mu_k)/\sigma_k \leq t}$ be the empirical distribution function of the standardized observed continuous variable $X_k$. In order to estimate the threshold parameters $\xi^j$ we need to evaluate percentiles of $\Tilde{X}_k$, i.e, $\hat{\xi}_r^j = \Tilde{\Xi}^{-1}(p_{\cdot r}^j)$. As the $X_k$'s are unique with probability tending to one, we can estimate the corresponding percentiles by means of order statistics, i.e.  
%\begin{equation*}
%    \Tilde{\Xi}^{-1}\Big(\frac{c}{n}\Big) = \frac{(X_{k(c+1)} - X_{k(c)})/2 - %\Bar{X}_k}{\hat{\sigma}_k} \quad \text{for } c= 0,1, \dots, n,
%\end{equation*}
%where $X_{k(c)}$ denotes the $c^{th}$ order statistic of the random variable $X_k$ \citep[see][for more details]{David03}. Furthermore, we set $\Tilde{\Xi}^{-1}\Big(\frac{c}{n}\Big)$ to $-\infty$ and $+\infty$ for $c=0$ and $c=n$, respectively. 

%Finally, we can construct a sample estimate of $\lambda(\Xi; x^j, \xi^j)$, namely:
%\begin{equation*}
%    \lambda(\Tilde{\Xi}; x^j, \hat{\xi}^j) = \sum_{r=1}^{l_{X_j}} x_r^j \int_{\hat{\xi}_{r-1}^{j}}^{\hat{\xi}_{r}^{j}} z d\Tilde{\Xi}(z) = \frac{1}{n\hat{\sigma}_k}\sum_{r=1}^{l_{X_j}} x_r^j \sum_{c=1+np_{\cdot r-1}^j}^{np_{\cdot r-1}^j}(X_{k(c)} - \Bar{X}_k),
%\end{equation*}
%where clearly equality only holds when the $X_k$'s are all distinct. In practice, when ties are present we still use the right-most summation expression and make sure that null sums are set to zero whenever $p_{\cdot r-1}^j = p_{\cdot r}^j$. Let us denote the resulting estimator 
%\begin{equation}\label{eq:brogden_adhoc}
%     \hat{\Sigma}_{jk}^+ = \frac{\hat{\rho}^{\text{\textit{Sp}}}_{jk} \hat{\sigma}_j}{\lambda(\Tilde{\Xi}; x^j, \hat{\xi}^j)}.
%\end{equation}
...
%However, $\hat{\Sigma}_{jk}^+$ is unchanged by location and scale transformation i.e. $a + bX_j$ and $a + bX_k$ only so long as $b > 0$. What's more, $\hat{\Sigma}_{jk}^+$ need not be bounded by $-1$ from below. Therefore, we simply let $\hat{\Sigma}_{jk}^- = - \hat{\Sigma}_{j-k}^+$ where $\hat{\Sigma}_{j-k}^+$ indicates that we take $- X_k = -(X_{k1}, \dots X_{kn})^\prime$ rather than $X_k$. Finally, we can state the generalized ad hoc estimator, i.e.
...
\begin{condition}[Hessian statistical noise]\label{Hessian statistical noise}
    ...
    \end{equation*}
    Again, $\ell_{jk}$ corresponds to the respective log-likelihood functions in Definitions 2.3 and 2.4.% \ref{definition_case2} and \ref{definition_case3}. 
    ...
    \begin{equation*}
        ...
        \end{split} 
    \end{equation*}
    due to Assumptions 3.1 and 3.2 % \ref{ass1} and \ref{ass2} %
...
\begin{condition}[Hessian regularity]
    ...
    \begin{condition}[Population risk is strongly Morse]
        ...
        %Clearly, for all critical points $\Sigma_{jk} \neq \Sigma_{jk}^*$ Assumption \ref{ass3} readily implies both conditions. Therefore, it remains to show that $\Sigma_{jk}^*$ is a non-degenerate critical point.
        
        
        
        Let us verify that $R^{\prime\prime}(\Sigma_{jk}) \neq 0$ for cases II and III. Indeed by Lemma \ref{expectation_commutes_case2} and Corollary \ref{expectation_commutes_case3} we can rewrite $R^{\prime\prime}(\Sigma_{jk})$ and obtain 
        \begin{align*}
            R^{\prime\prime}(\Sigma_{jk}) &= \mathbb{E}_{\Sigma_{jk}}\Bigg[\frac{\partial^2 \ell(\Sigma_{jk})}{\partial\Sigma_{jk}^2}\Bigg] \\ 
            &= \mathbb{E}_{\Sigma_{jk}^*} \begin{cases}
            \frac{\partial^2 p(x^j_{r} \mid x_{k}, \Sigma_{jk})/\partial \Sigma_{jk}^{2}}{p(x^j_{r} \mid x_{k}, \Sigma_{jk})} - \Bigg(\frac{\partial p(x^j_{r} \mid x_{k}, \Sigma_{jk}) / \partial \Sigma_{jk}}{p(x^j_{r} \mid x_{k}, \Sigma_{jk})}\Bigg)^2 & \text{for case II}, \\
            \frac{\partial^2 \pi(\Sigma_{jk})_{rs}/\partial \Sigma_{jk}^{2}}{\pi(\Sigma_{jk})_{rs}} - \Bigg(\frac{\partial \pi(\Sigma_{jk})_{rs}/\partial \Sigma_{jk}}{\pi(\Sigma_{jk})_{rs}}\Bigg)^2 & \text{for case III},
            \end{cases}
        \end{align*}
        where in $\pi(\Sigma_{jk})_{rs}$ we made the dependence on $\Sigma_{jk}$ explicit. 
        Note, that for case II we have
        \begin{multline*}
                \mathbb{E}_{\Sigma_{jk}^*}\Bigg[\frac{\partial^2 p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)/\partial \Sigma_{jk}^{2}}{p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)}\Bigg] \\
                \begin{aligned}
                &= \int_{-\infty}^{\infty} \sum_{r=1}^{l_{l_{X_j}}} \frac{\partial^2 p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)/\partial \Sigma_{jk}^{2}}{p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)} p(x^j_{r}, x_{k}; \Sigma_{jk}^*) dx_k \\
                &= \int_{-\infty}^{\infty} \sum_{r=1}^{l_{l_{X_j}}} \frac{\partial^2 p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)/\partial \Sigma_{jk}^{2}}{p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)} p(x^j_{r} \mid x_{k}, \Sigma_{jk}^*)p(x_{k}) dx_k \\
...
With these considerations, we have verified the required four conditions to hold such that Theorem 3.2
%\ref{uniform_convergence}%
...
\begin{lemma}
    \begin{proof}
        Recall that
        \begin{equation}
            \hat{\gamma}_r^j = \Phi^{-1}(p_{\cdot r}^j),
        \end{equation}
        where $p_{\cdot r}^j$ denotes the proportion of observing $x_r^j$ in an n-sample of the random variable $X_j$ where $1 < j < d_1+1$, i.e.
        \begin{equation*}
            p_{\cdot r}^j = \frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x_r^j)}.
        \end{equation*}
        First, note that $\Phi^{-1}(\cdot)$ is Lipschitz on $[\Phi(-2G),\Phi(2G)]$ with a Lipschitz constant $L_1$ such that under the event $A_r^j = \{\abs{\hat{\gamma}^j_r} \leq 2G\}$
        \begin{equation*}
            \begin{split}
                \abs{\hat{\gamma}_r^j - \gamma_r^j} &= \abs{\Phi^{-1}(\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x_r^j)}) - \Phi^{-1}(\Phi(\gamma_r^j))} \\
                &\leq L_1\abs{\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x_r^j)} - \Phi(\gamma_r^j)}.
            \end{split}
        \end{equation*}
        Now, we are interested in $\phi(\hat{\gamma}^j_r)$, therefore note further that $\phi(\cdot)$, namely the density function of the standard normal is Lipschitz with Lipschitz constant $L_0 = (2\pi)^{-1/2}e^{-1/2}$, s.t.
        \begin{equation*}
            \abs{\phi(\hat{\gamma}_r^j) - \phi(\gamma_r^j)} \leq L_0 \abs{\hat{\gamma}_r^j - \gamma_r^j} \leq \abs{\hat{\gamma}_r^j - \gamma_r^j},
        \end{equation*}
        as $L_0 < 1$. 
        We may then bound the probability of the complementary event as
        \begin{equation*}
        \begin{split}
            P({A^c}_{r}^{j}) &= P\Big(\abs{\hat{\gamma}^j_r} > 2G\Big) \\
                             &= P\Big(\abs{\hat{\gamma}^j_r} - \abs{\gamma^j_r} > 2G - \abs{\gamma^j_r}\Big) \\
                             &\leq P\Big(\abs{\hat{\gamma}^j_r} - \abs{\gamma^j_r} > G\Big) \quad \text{(Assumption \ref{ass2})} \\
                             &\leq P\Big(\abs{\hat{\gamma}^j_r - \gamma^j_r} > G\Big) \quad \text{(reverse triangle inequality)} \\
                             &\leq P\Big(\abs{\frac{1}{n} \sum_{i=1}^n \mathbbm{1}{(X_{ji} = x_r^j)} - \Phi(\gamma_r^j)} > \frac{G}{L_1}\Big) \\ 
                             &\leq 2\exp{\Big(- \frac{2G^2n}{L_1^2}\Big)},
        \end{split} 
        \end{equation*}
        where the last step follows from Hoeffding's inequality. Further define the event $A^j = \bigcap_{r=1}^{l_{X_j}-1} A_r^j$ and observe that
        \begin{equation*}
            P({A^c}^j) = P\Big(\bigcup_{r=1}^{l_{X_j}-1} {A^c}_r^j\Big) \leq \sum_{r=1}^{l_{X_j}-1} P({A^c}_r^j) \leq 2(l_{X_j}-1)\exp{\Big(- \frac{2G^2n}{L_1^2}\Big)}.
        \end{equation*}
    \end{proof}
\end{lemma}    
...
\end{equation*}
as $L_0 < 1$. Consequently, the statements regarding accuracy of the threshold estimates in Section 3.4 %\ref{sec::thresholds} %
still hold here. 
...
Let us assume w.l.o.g. that $X_j$ -- the discrete variable -- to have zero mean and variance one. As a consequence, we would simply need to adjust the integer scoring of the threshold estimates i.e.
\begin{equation*}
    \sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}^j_r)(x^j_{r+1} - x^j_r).
\end{equation*}


Therefore, the true covariance of the pair is bounded from above by the Cauchy-Schwarz inequality, i.e.
\begin{equation*}
    \abs{S^*_{f(X_k)X_j}} \leq \sigma^{2}_{f(X_k)} \sigma^{2}_{X_j} = 1.
\end{equation*}
...
\begin{equation*}
    P\left(\max_{jk} \abs{S_{\hat{f}(X_k)X_j} - S_{{f}(X_k)X_j}} \geq 2\epsilon \right) = o(1).
\end{equation*}

As \citet{Ravikumar11} show in Lemma 1 with sub-Gaussian parameter $c > 0$ ($f(X_k)$ is standard Gaussian and thus clealy also sub-Gaussian and $X_j$ is discrete and bounded and therefore also sub-Gaussian.) we have the following tail bound
\begin{equation*}
    P\left(\max_{jk}\abs{S_{{f}(X_k)X_j} - S^*_{{f}(X_k)X_j}} \geq \epsilon \right) \leq 4d^2 \exp\left\{- \frac{n\epsilon^2}{128(1+4c^2)^2} \right\},
\end{equation*}
for all $\epsilon \in (0,8(1+4c^2))$.

In similar spirit, we have already argued that 

\begin{equation*}
    P\left(\max_{k} \abs{\sigma^{(n)}_{\hat{f}(X_k)} - \sigma^{(n)}_{{f}(X_k)}} \geq 2\epsilon \right) = o(1).
\end{equation*}

Let us show that 

\begin{equation*}
    P\left(\max_{jk} \abs{\sigma^{(n)}_{{f}(X_k)} - 1 }\geq \epsilon \right) = o(1).
\end{equation*}

%First, let us assume that the latent sample variance is bounded away from infinity, i.e. lets assume $\text{Var}^{(n)}(f(X_k)) \leq b$ for all $k \in d_1+1, \dots, d_2$. Then by the fact that $(\sigma^{(n)}_{{f}(X_k)})^2 = \frac{1}{n-1} \sum_{i=1}^n (f(X_{ik}) - \mu_n(f))^2 $ is a U-Statistic with bounded kernel we get immediately get the following concentration inequality:

We may use the same Lemma 1 in \citet{Ravikumar11} to bound the variance, the diagonal entries in the covariance matrix. Since $f(X_k)$ is standard Gaussian and hence sub-Gaussian with parameter $c = 1$ we immediately get
\begin{equation*}
    P\left(\max_k \abs{(\sigma^{(n)}_{{f}(X_k)})^2 - 1} \geq \epsilon \right) \leq 4d_2\exp \left\{- \frac{n\epsilon^2}{128(1+4)^2} \right\}.
\end{equation*}
Put differently, with high probability 
\begin{equation*}
    (\sigma^{(n)}_{{f}(X_k)})^2 \in [1 - \epsilon, 1+ \epsilon],
\end{equation*}
and consequently we also have with high probability 
\begin{equation*}
    \sigma^{(n)}_{{f}(X_k)} \in [\sqrt{1 -\epsilon}, \sqrt{1+\epsilon} ].
\end{equation*}
Since the interval $[1 -\epsilon, 1+\epsilon ]$ is always as least as wide as $[\sqrt{1 -\epsilon}, \sqrt{1+\epsilon} ]$, for all $\epsilon >0$ with high probability we then also have
\begin{equation*}
    \sigma^{(n)}_{{f}(X_k)} \in [1 -\epsilon, 1+\epsilon ].
\end{equation*}

We have shown that for some $\epsilon>0$,
\begin{equation*}
    S_{\hat{f}(X_k)X_j} \in [S_{{f}(X_k)X_j} - 2\epsilon , S_{{f}(X_k)X_j} + 2\epsilon] \qquad \forall j,k,
\end{equation*}
and 
\begin{equation*}
    \sigma^{(n)}_{\hat{f}(X_k)} \in [\sigma^{(n)}_{{f}(X_k)} - 2\epsilon , \sigma^{(n)}_{{f}(X_k)} + 2\epsilon] \qquad \forall j,k,
\end{equation*}
with high probability. 
Furthermore, 
\begin{equation*}
    S_{{f}(X_k)X_j} \in [-1 - \epsilon , 1 + \epsilon] \qquad \forall j,k,
\end{equation*}
and 
\begin{equation*}
    \sigma^{(n)}_{{f}(X_k)} \in [1 - \epsilon , 1 + \epsilon] \qquad \forall j,k,
\end{equation*}
with high probability. 

Thus we have for $\epsilon \in (0,1/4]$ at worst
\begin{equation*}
    S_{\hat{f}(X_k)X_j} \in [-1-\epsilon - 2\epsilon , 1 + \epsilon + 2\epsilon] = [-1.75,1.75] \qquad \forall j,k,
\end{equation*}
and similarly
\begin{equation*}
    \sigma^{(n)}_{\hat{f}(X_k)} \in [1-\epsilon - 2\epsilon , 1 + \epsilon + 2\epsilon] = [.25,1.25] \qquad \forall j,k,
\end{equation*}


Now, consider the following function $h: \mathbb{R}\times \mathbb{R}_+ \to \mathbb{R}$ defined by
            \begin{equation}
                h(u,v) = \frac{u}{v},
            \end{equation}
with $\nabla h = (1/v, -u/v^{2})^{T}$.
...
\begin{equation*}
   \sup \norm{\nabla h}_2 = \sqrt{\left(\frac{1}{0.25}\right)^2 + \left(\frac{-1.75}{(0.25)^2}\right)^2} \coloneqq L.
\end{equation*}
...
Consider
\begin{multline*}
    P\left(\max_{jk}\abs{\hat{\Sigma}_{jk}^{(n)} -  \Sigma_{jk}^{*}} \geq 2\epsilon \right) \\
    \begin{aligned}
    &\leq P\left( \max_{jk} \abs{\frac{S_{\hat{f}(X_k)X_j}}{\sigma^{(n)}_{\hat{f}(X_k)}\sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}^j_r)} - \frac{S_{{f}(X_k)X_j}}{\sigma^{(n)}_{{f}(X_k)}\sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}^j_r)}} \geq \epsilon \right) \\
    &+ P\left(\max_{jk} \abs{\frac{S_{{f}(X_k)X_j}}{\sigma^{(n)}_{{f}(X_k)}\sum_{r=1}^{l_{X_j}-1} \phi(\bar{\gamma}^j_r)} - \Sigma_{jk}^{*}} \geq \epsilon \right) \\
    &\leq P\left(\max_{jk} \abs{S_{\hat{f}(X_k)X_j} - S_{{f}(X_k)X_j}}  \geq \frac{\epsilon}{2L} \right) + P\left(\max_{jk} \abs{S_{{f}(X_k)X_j} - S^*_{{f}(X_k)X_j}}  \geq \frac{\epsilon}{2L} \right) \\
    &+ P\left(\max_{k} \abs{\sigma^{(n)}_{\hat{f}(X_k)} - \sigma^{(n)}_{{f}(X_k)}} \geq \frac{\epsilon}{2L} \right) + P\left(\max_{k} \abs{\sigma^{(n)}_{{f}(X_k)} - 1} \geq \frac{\epsilon}{2L} \right)
    \end{aligned}
\end{multline*}
