...
\begin{figure}\label{fig:case2_speed}
    ...
    \caption{Computation time in milliseconds for the \textit{Case II} MLE and ad-hoc estimators. Depicted are median (solid line), first and third quartile (shaded area) computation time. In the left panel time is compared against a grid of sample sizes \(n \in [50, 10000]\) with a step size of \(s_t = 50\). In the right panel, time is compared against a length-\(200\) grid of true correlation values \(\Sigma^*_{jk} \in [-.98, .98]\).}
\end{figure}

The right panel of \figref{fig:case2_speed} demonstrates computation time across a grid of length \(200\) of values for \(\Sigma_{jk}^* \in [-.98, 98]\). Sample size is in this case fixed at \(n=1000\). The ad-hoc estimator is consistently and considerably faster than the MLE regardless of which strategy is used. The difference in computation time is especially pronounced for large sample sizes and large correlation values. Setting the FOC to zero and solving for \(\Sigma_{jk}\) is computationally more efficient than directly maximizing the log-likelihood function. The time difference in MLE strategies is more pronounced at the end-points of the $[-1,1]$ interval. The ad-hoc estimator is not affected by this issue. In the high-dimensional setting we consider in this paper therefore, the ad-hoc estimator is preferable to the MLE due to its computational efficiency, it simplicity which allows us to form concentration inequalities and its robustness to the underlying correlation value.
