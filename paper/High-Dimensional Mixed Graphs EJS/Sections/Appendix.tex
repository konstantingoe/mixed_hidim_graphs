...
%We need to find sample equivalences for each of the components in Eq. \eqref{eq:polyserial_adhoc}. As the numerator does not change as compared to the naive approach let us focus on $\lambda(\Xi; x^j, \xi^j)$. First, let $\tilde{\Xi}(t) = 1/n\sum_{i=1}^n \mathbbm{1}_{(X_k - \mu_k)/\sigma_k \leq t}$ be the empirical distribution function of the standardized observed continuous variable $X_k$. In order to estimate the threshold parameters $\xi^j$ we need to evaluate percentiles of $\tilde{X}_k$, i.e, $\hat{\xi}_r^j = \tilde{\Xi}^{-1}(p_{\cdot r}^j)$. As the $X_k$'s are unique with probability tending to one, we can estimate the corresponding percentiles using order statistics, i.e.
%\begin{equation*}
