...
\begin{lemma}\label{lemma::thresholds}
    Suppose the estimated thresholds are bounded away from infinity, i.e. \(\Abs{\hat\Gamma^r_j} \leq 2G\)\todo{Or just G?} for all $j \in [d_1]$ and $r = 1, \dots, l_j$ and some \(G\). The following bound holds for some Lipschitz constant $L_1$ and all $t > 0$:
    \begin{equation*}
