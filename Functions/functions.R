...
mixed.undir.graph <- function(data = data, verbose = T, nlam = 50, thresholding = F, param = .1) {
  ...
  if (any(abs(pair) > .9)) {
    sapply(seq_along(rho[lower.tri(rho)][which(abs(pair) > .95)]), function(k) {
      warning(
        paste0(
  ...
  output <- list(
    "Estimated Precision Matrix" = Omega_hat, "Adjacency Matrix" = adj_estimate,
...
glasso.results <- function(Sigma = Sigma, Omega = Omega, nlam = nlam, n = n, matexport = matexport, thresholding = F, param = param) {
  ...
  if (matexport == T) {
    output <- list(
      "True Precision Matrix" = Omega, "Estimated Precision Matrix" = Omega_hat, "Frobenius norm" = frobenius,
...
kendalls.results <- function(result_object = result_object) {
  table <- rbind(
    c(
      "kendall" = mean(sapply(1:sim, function(k) result_object[[k]][[1]][["Frobenius norm"]])), "kendall_sd" = sd(sapply(1:sim, function(k) result_object[[k]][[1]][["Frobenius norm"]])),
    ...
    c(
      mean(sapply(1:sim, function(k) result_object[[k]][[1]][["FPR"]])), sd(sapply(1:sim, function(k) result_object[[k]][[1]][["FPR"]])),
      mean(sapply(1:sim, function(k) result_object[[k]][[2]][["FPR"]])), sd(sapply(1:sim, function(k) result_object[[k]][[2]][["FPR"]]))
    ),
    c(
      mean(sapply(1:sim, function(k) result_object[[k]][[1]][["TPR"]])), sd(sapply(1:sim, function(k) result_object[[k]][[1]][["TPR"]])),
      mean(sapply(1:sim, function(k) result_object[[k]][[2]][["TPR"]])), sd(sapply(1:sim, function(k) result_object[[k]][[2]][["TPR"]]))
    ),
    c(
      mean(sapply(1:sim, function(k) result_object[[k]][[1]][["AUC"]])), sd(sapply(1:sim, function(k) result_object[[k]][[1]][["AUC"]])),
...
results_generator <- function(results_object) {
  table <- round(as_tibble(rbind(
    c(
      "latent" = mean(sapply(1:sim, function(k) results_object[[k]]$latent[["Frobenius norm"]])),
    ...
    c(
      mean(sapply(1:sim, function(k) results_object[[k]]$latent[["FPR"]])),
    ...
    c(
      mean(sapply(1:sim, function(k) results_object[[k]]$latent[["TPR"]])),
    ...
    c(
      mean(sapply(1:sim, function(k) results_object[[k]]$latent[["AUC"]])),
...
extract.ternary.results <- function(object) {
  table <- cbind(
    c(
      mean(sapply(1:sim, function(k) object[[k]][[1]][[1]])),
      sd(sapply(1:sim, function(k) object[[k]][[1]][[1]]))
    ),
    c(
      mean(sapply(1:sim, function(k) object[[k]][[2]][[1]])),
      sd(sapply(1:sim, function(k) object[[k]][[2]][[1]]))
    ),
    c(
      mean(sapply(1:sim, function(k) object[[k]][[3]][[1]])),
      sd(sapply(1:sim, function(k) object[[k]][[3]][[1]]))
    ),
    c(
      mean(sapply(1:sim, function(k) object[[k]][[4]][[1]])),
  ...
  for (i in 2:4) {
    table <- rbind(
      ...
      cbind(
        c(
          mean(sapply(1:sim, function(k) object[[k]][[1]][[i]])),
          sd(sapply(1:sim, function(k) object[[k]][[1]][[i]]))
        ),
        c(
          mean(sapply(1:sim, function(k) object[[k]][[2]][[i]])),
          sd(sapply(1:sim, function(k) object[[k]][[2]][[i]]))
        ),
        c(
          mean(sapply(1:sim, function(k) object[[k]][[3]][[i]])),
          sd(sapply(1:sim, function(k) object[[k]][[3]][[i]]))
        ),
        c(
          mean(sapply(1:sim, function(k) object[[k]][[4]][[i]])),
...
extract.kendall.nonpararesults <- function(object) {
  table <- cbind(
    ...
    c(
      mean(sapply(1:sim, function(k) object[[k]][[5]][[1]])),
  ...
  for (i in 2:4) {
    table <- rbind(
      ...
      cbind(
        ...
        c(
          mean(sapply(1:sim, function(k) object[[k]][[5]][[i]])),
...
adhoc_lord <- function(x, y, maxcor = 0.9999, more_verbose = F) {
  ...
  if (sum(factor_id) == 2) {
    ...
    lambda <- as.numeric(values %*% sapply(
      seq_along(values),
...
adhoc_lord_sim <- function(cont, disc, maxcor = 0.9999) {
  ...
  if (is.null(corr.hat)) {
    corr.hat <- polycor::polyserial(cont, disc)
  }
  return(corr.hat)
}
...
fan.case.3 <- function(x, y) {
  ...
  hatR <- tryCatch(
    expr = {
      optimize(bridge.func.case1, lower = -0.999, upper = 0.999, tol = 1e-8)[1]
    },
    error = function(e) {
...
fan.case.2 <- function(x, y) {
  ...
  hatR <- tryCatch(
    expr = {
      optimize(bridge.func.case2, lower = -0.999, upper = 0.999, tol = 1e-8)[1]
    },
    error = function(e) {
      # message('Caught an error!')
      # message(e)
...
ternery.cont <- function(cont, ter) {
  ...
  hatR <- tryCatch(
    expr = {
      optimize(bridge.30, lower = -0.999, upper = 0.999, tol = 1e-8)[1]
    },
    error = function(e) {
      # message('Caught an error!')
      # message(e)
...
ternery.ternery <- function(ter1, ter2) {
  ...
  hatR <- tryCatch(
    expr = {
      optimize(bridge.33, lower = -0.999, upper = 0.999, tol = 1e-8)[1]
    },
    error = function(e) {
      # message('Caught an error!')
      # message(e)
...
ternery.binary <- function(ter, bin) {
  ...
  hatR <- tryCatch(
    expr = {
      optimize(bridge.32, lower = -0.999, upper = 0.999, tol = 1e-8)[1]
    },
    error = function(e) {
      # message('Caught an error!')
      # message(e)
